export const tr = {
  common: {
    siteName: "AÜ Öğrenci Platformu",
    menu: {
      university: "Üniversitemiz",
      dormitory: "Yurt ve Ulaşım",
      notes: "Not Paylaşımı",
      events: "Etkinlikler",
      amasya: "Amasyamız",
      menu: "Menü",
      openMenu: "Menüyü aç",
      generalInfo: "Genel Bilgi",
      generalInfoDesc: "Üniversitemiz hakkında detaylı bilgi alın",
      faculties: "Fakülteler",
      facultiesDesc: "Fakültelerimiz ve bölümlerimiz",
      calendar: "Akademik Takvim",
      calendarDesc: "Akademik yıl planı ve önemli tarihler",
      dorms: "Yurtlar",
      dormsDesc: "Yurt imkanları ve konaklama seçenekleri",
      busRoutes: "Otobüs Güzergahları",
      busRoutesDesc: "Kampüs ulaşım hatları",
      busSchedule: "Sefer Saatleri",
      busScheduleDesc: "Güncel otobüs saatleri",
      historicalPlaces: "Tarihi Yerler",
      historicalPlacesDesc: "Amasya'nın tarihi ve kültürel mirası",
      tourRoutes: "Gezi Rotaları",
      tourRoutesDesc: "Önerilen gezi güzergahları",
      localFood: "Yöresel Lezzetler",
      localFoodDesc: "Amasya'nın eşsiz mutfağı",
      eventManagement: "Etkinlik Yönetimi"
    },
    auth: {
      login: "Giriş Yap",
      register: "Kayıt Ol",
      logout: "Çıkış Yap"
    },
    buttons: {
      explore: "Üniversitemizi Keşfet",
      startSharing: "Not Paylaşımına Başla",
      moreInfo: "Daha Fazla Bilgi",
      viewFaculties: "Fakülteleri İncele",
    },
  },
  home: {
    hero: {
      title: "Amasya Üniversitesi Öğrenci Platformu",
      subtitle: "Öğrenci hayatınızı kolaylaştıran dijital çözümler",
    },
    features: {
      title: "Öne Çıkan Özellikler",
      subtitle: "Üniversite hayatınızı kolaylaştıracak tüm araçlar tek bir platformda",
      university: {
        title: "Üniversite Bilgileri",
        description: "Kampüs, fakülte ve bölümler hakkında detaylı bilgiler",
      },
      notes: {
        title: "Not Paylaşımı",
        description: "Ders notlarını paylaş, arkadaşlarınla işbirliği yap",
      },
      events: {
        title: "Etkinlikler",
        description: "Kampüsteki etkinlikleri takip et, katılım sağla",
      },
    },
    about: {
      title: "Amasya Üniversitesi Hakkında",
      description1: "Amasya Üniversitesi, 2006 yılında kurulmuş olup, köklü geçmişi ve modern eğitim anlayışıyla öğrencilerine kaliteli eğitim sunmaktadır.",
      description2: "11 Fakülte, 3 Enstitü, 1 Yüksekokul ve 7 Meslek Yüksekokulu ile geniş bir akademik yapıya sahiptir.",
    },
    ai: {
      title: "Amasya Rehber Chatbot",
      subtitle: "Amasya hakkında merak ettiğiniz her şeyi yapay zeka asistanımıza sorabilirsiniz",
      guide: {
        title: "Amasya Rehberi",
        description: "Tarihi yerler, gezilecek noktalar, yöresel lezzetler ve daha fazlası hakkında anlık bilgi alın",
        button: "Rehbere Sor"
      }
    },
    tourism: {
      title: "Amasya'yı Keşfedin",
      description1: "Binlerce yıllık tarihi, eşsiz mimarisi ve doğal güzellikleriyle Amasya'yı keşfedin.",
      description2: "Şehzadeler şehri Amasya'da gezilecek yerler, yöresel lezzetler ve konaklama seçenekleri hakkında detaylı bilgiler.",
      features: {
        locations: "Tarihi Yerler",
        routes: "Gezi Rotaları",
        cuisine: "Yöresel Lezzetler",
        accommodation: "Konaklama"
      },
      buttons: {
        explore: "Tarihi Yerleri Keşfet",
        routes: "Gezi Rotalarını İncele"
      }
    }
  },
  university: {
    generalInfo: {
      title: "Amasya Üniversitesi",
      subtitle: "Köklü geçmişi ve modern eğitim anlayışıyla geleceğin liderlerini yetiştiren, bilimsel araştırmaları ve toplumsal katkılarıyla öne çıkan bir yükseköğretim kurumu.",
      history: {
        title: "Tarihçe",
        content1: "Amasya Üniversitesi, 17 Mart 2006 tarihinde 5467 sayılı kanunla kurulmuştur. Üniversitemiz, Osmanlı döneminden bu yana eğitim ve kültür merkezi olan Amasya'nın bu geleneğini sürdürmektedir.",
        content2: "Kuruluşundan bu yana hızla büyüyen üniversitemiz, Amasya'nın farklı noktalarında yer alan kampüsleriyle eğitim-öğretim faaliyetlerini sürdürmektedir. Ana yerleşkemiz İpekköy'de bulunmaktadır.",
        content3: "Üniversitemiz, bilimsel araştırma ve geliştirme faaliyetleriyle bölgesel kalkınmaya katkı sağlamakta, ulusal ve uluslararası işbirlikleriyle akademik gelişimi desteklemektedir."
      },
      statistics: {
        title: "Sayılarla Amasya Üniversitesi",
        students: "Öğrenci Sayısı",
        academicStaff: "Akademik Personel",
        administrativeStaff: "İdari Personel ve Sürekli İşçi",
        faculties: "Fakülte Sayısı",
        institutes: "Enstitü Sayısı",
        schools: "Yüksekokul Sayısı",
        vocationalSchools: "Meslek Yüksekokulu",
        researchCenters: "Araştırma Merkezi",
        graduates: "Mezun Sayısı"
      },
      academicStructure: {
        title: "Akademik Yapı",
        faculties: {
          title: "Fakültelerimiz",
          subtitle: "Amasya Üniversitesi'nin akademik birimlerini keşfedin",
          departments: "Bölümler",
          list: {
            education: {
              name: "Eğitim Fakültesi",
              description: "Geleceğin eğitimcilerini yetiştiren fakültemiz",
              departments: [
                "Matematik ve Fen Bilimleri Eğitimi",
                "Temel Eğitim",
                "Türkçe ve Sosyal Bilimler Eğitimi",
                "Özel Eğitim",
                "Eğitim Bilimleri",
                "Yabancı Dil Eğitimi"
              ]
            },
            arts: {
              name: "Fen Edebiyat Fakültesi",
              description: "Temel bilimler ve sosyal bilimler alanında eğitim veren fakültemiz",
              departments: [
                "Matematik",
                "Fizik",
                "Kimya",
                "Biyoloji",
                "Türk Dili ve Edebiyatı",
                "Tarih",
                "Coğrafya"
              ]
            },
            fineArts: {
              name: "Hattat Hamdullah Güzel Sanatlar Fakültesi",
              description: "Sanat ve tasarım alanında eğitim veren fakültemiz",
              departments: [
                "Geleneksel Türk Sanatları",
                "Resim",
                "Grafik Tasarım"
              ]
            },
            theology: {
              name: "İlahiyat Fakültesi",
              description: "İslami ilimler alanında eğitim veren fakültemiz",
              departments: [
                "İlahiyat"
              ]
            },
            economics: {
              name: "Merzifon İktisadi ve İdari Bilimler Fakültesi",
              description: "İktisat, işletme ve yönetim alanlarında eğitim veren fakültemiz",
              departments: [
                "İşletme",
                "İktisat",
                "Siyaset Bilimi ve Kamu Yönetimi",
                "Uluslararası İlişkiler"
              ]
            },
            architecture: {
              name: "Mimarlık Fakültesi",
              description: "Mimarlık ve şehir planlama alanında eğitim veren fakültemiz",
              departments: [
                "Mimarlık",
                "Şehir ve Bölge Planlama"
              ]
            },
            engineering: {
              name: "Mühendislik Fakültesi",
              description: "Mühendislik alanında eğitim veren fakültemiz",
              departments: [
                "Bilgisayar Mühendisliği",
                "Elektrik-Elektronik Mühendisliği",
                "İnşaat Mühendisliği",
                "Makine Mühendisliği"
              ]
            },
            health: {
              name: "Sağlık Bilimleri Fakültesi",
              description: "Sağlık bilimleri alanında eğitim veren fakültemiz",
              departments: [
                "Hemşirelik",
                "Ebelik",
                "Beslenme ve Diyetetik",
                "Fizyoterapi ve Rehabilitasyon"
              ]
            },
            medicine: {
              name: "Tıp Fakültesi",
              description: "Tıp eğitimi veren fakültemiz",
              departments: [
                "Tıp"
              ]
            }
          }
        },
        institutes: {
          title: "Enstitüler",
          items: [
            "Fen Bilimleri Enstitüsü",
            "Sağlık Bilimleri Enstitüsü",
            "Sosyal Bilimler Enstitüsü"
          ]
        },
        schools: {
          title: "Yüksekokullar",
          items: [
            "Yabancı Diller Yüksekokulu"
          ]
        },
        vocationalSchools: {
          title: "Meslek Yüksekokulları",
          items: [
            "Gümüşhacıköy Hasan Duman MYO",
            "Merzifon Meslek Yüksekokulu",
            "Sabuncuoğlu Şerefeddin Sağlık Hizmetleri MYO",
            "Sosyal Bilimler Meslek Yüksekokulu",
            "Suluova Meslek Yüksekokulu",
            "Tasarım Meslek Yüksekokulu",
            "Taşova Yüksel Akın Meslek Yüksekokulu",
            "Teknik Bilimler Meslek Yüksekokulu"
          ]
        }
      },
      mission: {
        title: "Misyonumuz",
        content: "Evrensel değerler ışığında, bilgi ve teknolojiyi kullanabilen, araştırmacı, katılımcı, çözüm odaklı, etik değerlere sahip, nitelikli bireyler yetiştirmek ve bölgesel kalkınmaya katkı sağlamaktır."
      },
      vision: {
        title: "Vizyonumuz",
        content: "Eğitim, araştırma ve topluma hizmet alanlarında ulusal ve uluslararası düzeyde tanınan, tercih edilen, öncü bir üniversite olmaktır."
      }
    }
  },
  dormitories: {
    title: "KYK Yurtları",
    subtitle: "Amasya'daki tüm KYK yurtları hakkında detaylı bilgi",
    tabs: {
      all: "Tüm Yurtlar",
      male: "Erkek Yurtları",
      female: "Kız Yurtları"
    },
    info: {
      capacity: "Kapasite",
      occupancy: "Doluluk",
      location: "Konum",
      address: "Adres"
    },
    occupancyStatus: {
      full: "% Dolu",
      available: "Müsait",
      limited: "Az Yer",
      unavailable: "Dolu"
    },
    types: {
      male: "ERKEK",
      female: "KIZ"
    }
  },
  historicalPlaces: {
    title: "Tarihi Yerler",
    subtitle: "Amasya'nın Eşsiz Tarihi Mirası",
    period: {
      ancient: "Antik Dönem",
      ottoman: "Osmanlı Dönemi",
      seljuk: "Selçuklu Dönemi"
    },
    places: {
      pontusRoyalTombs: {
        title: "Pontus Kral Kaya Mezarları",
        period: "M.Ö. 4. Yüzyıl",
        description: "Amasya'nın Yeşilırmak vadisine bakan sarp kayalıklarında yükselen Pontus Kral Kaya Mezarları, yaklaşık 2300 yıl öncesine uzanan tarihiyle kentin en görkemli yapıları arasında yer alır. M.Ö. 4. yüzyılda Pontus Krallığı döneminde inşa edilen bu mezarlar, dönemin krallarına ait anıtsal birer eser olarak günümüze ulaşmıştır.\n\nKayalara oyulmuş bu devasa mezarlar, yalnızca tarihi bir miras değil, aynı zamanda Amasya'nın doğal güzelliğini taçlandıran bir estetik unsurdur. Akşam ışıklandırmalarıyla birlikte, Yeşilırmak'ın kıyısında büyüleyici bir manzara sunarak kente mistik bir hava katar. Tarih ve doğanın iç içe geçtiği bu eşsiz yapılar, Amasya'nın kültürel dokusunu tamamlayan en önemli öğelerden biridir.",
        highlights: [
          "2300 yıllık tarih",
          "Kayalara oyulmuş anıtsal yapı",
          "Gece ışıklandırması",
          "Yeşilırmak manzarası"
        ]
      },
      amasyaCastle: {
        title: "Amasya Kalesi (Harşena Kalesi)",
        period: "Hititler - Persler - Pontus Krallığı - Osmanlı",
        description: "Yeşilırmak Vadisi'ne hakim bir konumda yükselen Amasya Kalesi (Harşena Kalesi), binlerce yıllık geçmişiyle kentin en önemli tarihi yapılarından biridir. Stratejik konumu sayesinde Hititler'den Persler'e, Pontus Krallığı'ndan Osmanlı'ya kadar birçok medeniyetin hâkimiyetine giren kale, özellikle Pontus Krallığı döneminde büyük önem kazanmıştır. Adını, Pontus Kralı Mithridates'in komutanı Harşena'dan alan bu ihtişamlı yapı, Osmanlı döneminde de savunma amaçlı kullanılmıştır.\n\nHarşena Kalesi, yalnızca tarihi değeriyle değil, aynı zamanda şehre kattığı eşsiz manzarasıyla da büyüleyicidir. Zirvesinden bakıldığında Yeşilırmak, Pontus Kral Kaya Mezarları ve Amasya'nın tarihi evleri muhteşem bir uyum içinde gözler önüne serilir. Gün batımında altın sarısına bürünen kale ve çevresi, Amasya'nın doğal ve kültürel güzelliğini taçlandıran eşsiz bir silüet oluşturur.",
        highlights: [
          "Hititler - Persler - Pontus Krallığı - Osmanlı",
          "Stratejik konum",
          "Muhteşem manzara",
          "Tarih ve doğanın iç içe geçtiği eşsiz yapı"
        ]
      },
      bayezidComplex: {
        title: "II. Bayezid Külliyesi",
        period: "1485-1486",
        description: "Osmanlı sultanlarından II. Bayezid tarafından 1485-1486 yılları arasında yaptırılan Amasya II. Bayezid Külliyesi, şehrin en önemli Osmanlı eserlerinden biridir. Cami, medrese, imaret ve kütüphaneden oluşan bu görkemli yapı, dönemin mimari zarafetini yansıtan estetik detaylarıyla dikkat çeker. Özellikle sade ve dengeli mimarisi, iç mekândaki huzurlu atmosferiyle birleşerek ziyaretçilerine tarih içinde bir yolculuk sunar.\n\nYeşilırmak'ın kıyısında yer alan külliye, şehre kattığı zarafet ve tarihi dokuyla Amasya'nın siluetine ayrı bir güzellik kazandırır. Özellikle caminin iki zarif minaresi ve taş işçiliğindeki incelik, Osmanlı sanatının en güzel örneklerinden biri olarak karşımıza çıkar. Günün farklı saatlerinde değişen ışık oyunlarıyla külliye, Amasya'nın tarihî ve doğal güzelliğini tamamlayan eşsiz bir yapı olarak varlığını sürdürmektedir.",
        highlights: [
          "1485-1486",
          "Osmanlı sultanlarından II. Bayezid tarafından yaptırılan",
          "Cami, medrese, imaret ve kütüphaneden oluşan görkemli yapı",
          "Tarih ve doğanın iç içe geçtiği eşsiz yapı"
        ]
      },
      hazeranlarMansion: {
        title: "Hazeranlar Konağı",
        period: "19. Yüzyıl",
        description: "Yeşilırmak'ın kıyısında, tarihi Yalıboyu Evleri'nin en güzel örneklerinden biri olan Hazeranlar Konağı, 19. yüzyılda Defterdar Hasan Talat Efendi tarafından inşa ettirilmiştir. Osmanlı sivil mimarisinin zarafetini yansıtan bu yapı, adını burada yaşamış olan Hazeran Hanım'dan almıştır. Ahşap işçiliği, geniş avlusu ve simetrik mimarisiyle göz kamaştıran konak, geçmişin izlerini günümüze taşıyan önemli bir kültürel mirastır.\n\nBugün müze ve sanat galerisi olarak kullanılan Hazeranlar Konağı, Amasya'nın estetik ruhunu yansıtan yapılar arasında yer alır. Yeşilırmak ile tarihi dokunun kusursuz uyumu, ziyaretçilere hem görsel bir şölen sunar hem de Osmanlı döneminin günlük yaşamına dair ipuçları verir. Özellikle gece ışıklandırmalarıyla birlikte konak, şehrin siluetine nostaljik ve zarif bir güzellik katmaktadır.",
        highlights: [
          "19. Yüzyıl",
          "Defterdar Hasan Talat Efendi tarafından inşa edilmiş",
          "Osmanlı sivil mimarisinin zarafetini yansıtan yapı",
          "Gece ışıklandırması"
        ]
      },
      ferhatSirinWaterways: {
        title: "Ferhat ile Şirin Su Kanalları",
        period: "Rivayet",
        description: "Amasya'nın en romantik ve efsanevi yapılarından biri olan Ferhat ile Şirin Su Kanalları, halk arasında dilden dile aktarılan büyük bir aşk hikâyesinin izlerini taşır. Rivayete göre, Ferhat isimli bir nakkaş, Amasya Sultanı'nın kızı Şirin'e âşık olur. Ancak Sultan, bu aşka karşı çıkar ve Ferhat'a Şirin'e kavuşabilmesi için dağları delip şehre su getirmesi şartını koşar. Büyük bir aşkla işe koyulan Ferhat, keski ve çekiçle dağları delerken, Şirin'in öldüğüne dair yalan bir haber alır ve bu acıya dayanamayarak hayatına son verir.\n\nBugün hala görülebilen Ferhat Su Kanalları, yalnızca bir mühendislik harikası olarak değil, aynı zamanda aşkın ve azmin bir sembolü olarak Amasya'nın tarihi dokusuna katkı sunar. Dağların içinden geçen bu eski su yolu, şehrin doğal güzellikleriyle bütünleşerek ziyaretçilere hem romantik hem de mistik bir atmosfer sunar. Ferhat'ın aşkı ve emeğiyle şekillenen bu kanallar, Amasya'nın efsanelerle bezeli tarihine anlam katan en özel yapılar arasındadır.",
        highlights: [
          "Rivayet",
          "Amasya'nın en romantik ve efsanevi yapılarından biri",
          "Aşk hikâyesi",
          "Dağların içinden geçen eski su yolu"
        ]
      }
    }
  },
  localFood: {
    title: "Yöresel Lezzetler",
    subtitle: "Amasya'nın Eşsiz Mutfağı",
    tabs: {
      mainDishes: "Ana Yemekler",
      desserts: "Tatlılar"
    },
    details: {
      ingredients: "Malzemeler",
      prepTime: "Hazırlama Süresi",
      difficulty: "Zorluk",
      servings: "Porsiyon",
      minutes: "dakika"
    },
    difficulty: {
      easy: "Kolay",
      medium: "Orta",
      hard: "Zor"
    },
    dishes: {
      keskek: {
        name: "Keşkek",
        description: "Amasya'nın en meşhur yemeklerinden biri olan keşkek, düğün ve özel günlerde yapılan, dövme buğday ve et ile hazırlanan geleneksel bir yemektir.",
        ingredients: [
          "Dövme buğday",
          "Kuzu eti",
          "Tereyağı",
          "Tuz",
          "Karabiber"
        ],
        preparationTime: "3-4 saat",
        difficulty: "Orta",
        servings: "6-8 kişilik"
      },
      baklaDolmasi: {
        name: "Bakla Dolması",
        description: "Taze bakla içerisine pirinç, soğan ve çeşitli baharatlarla hazırlanan iç malzemesi doldurularak yapılan geleneksel bir Amasya yemeğidir.",
        ingredients: [
          "Taze bakla",
          "Pirinç",
          "Soğan",
          "Maydanoz",
          "Zeytinyağı",
          "Baharatlar"
        ],
        preparationTime: "1 saat",
        difficulty: "Orta",
        servings: "4-6 kişilik"
      },
      catalAsi: {
        name: "Çatal Aşı",
        description: "Yufka parçaları üzerine kıymalı sos ve yoğurt dökülerek hazırlanan, Amasya'ya özgü bir mantı çeşididir.",
        ingredients: [
          "Yufka",
          "Kıyma",
          "Soğan",
          "Yoğurt",
          "Tereyağı",
          "Baharatlar"
        ],
        preparationTime: "45 dakika",
        difficulty: "Kolay",
        servings: "4 kişilik"
      }
    },
    desserts: {
      cicekBamyasi: {
        name: "Amasya Çiçek Bamyası",
        description: "Amasya'nın meşhur tatlı çeşitlerinden biri olan çiçek bamyası, kurutulmuş bamyaların şerbetle tatlandırılmasıyla yapılır.",
        ingredients: [
          "Kurutulmuş çiçek bamya",
          "Şeker",
          "Su",
          "Limon"
        ],
        preparationTime: "30 dakika",
        difficulty: "Kolay",
        servings: "4-6 kişilik"
      },
      elmaTatlisi: {
        name: "Amasya Elması Tatlısı",
        description: "Amasya'nın dünyaca ünlü elmaları kullanılarak yapılan, şerbetli bir tatlı çeşididir.",
        ingredients: [
          "Amasya elması",
          "Şeker",
          "Su",
          "Tarçın",
          "Ceviz içi"
        ],
        preparationTime: "40 dakika",
        difficulty: "Kolay",
        servings: "6 kişilik"
      }
    }
  },
  notes: {
    title: "Not Paylaşımı",
    filters: {
      faculty: "Fakülte seçin",
      department: "Bölüm seçin",
      category: "Kategori seçin",
      search: "Ara...",
      clearAll: "Filtreleri Temizle",
      bookmarked: "Kaydedilenler",
      allNotes: "Tüm Notlar"
    },
    categories: {
      DERS_NOTU: "Ders Notu",
      SINAV_NOTU: "Sınav Notu",
      OZET: "Özet",
      KAYNAK: "Kaynak"
    },
    share: {
      button: "Not Paylaş",
      title: "Not Paylaş",
      form: {
        title: "Başlık",
        titlePlaceholder: "Not başlığı",
        faculty: "Fakülte",
        department: "Bölüm",
        category: "Kategori",
        content: "İçerik",
        contentPlaceholder: "Not içeriği",
        files: {
          title: "Dosyalar",
          supportedFormats: "Desteklenen formatlar: PDF, Word, Excel, PowerPoint, JPEG, PNG (max: 10MB)",
          invalidFile: "Geçersiz dosya formatı veya boyutu çok büyük (max: 10MB)"
        },
        submit: "Paylaş",
        submitting: "Paylaşılıyor..."
      }
    },
    detail: {
      notFound: "Not bulunamadı",
      delete: {
        title: "Notu Silmek İstediğinize Emin Misiniz?",
        description: "Bu işlem geri alınamaz. Not ve ilgili tüm yorumlar kalıcı olarak silinecektir.",
        cancel: "Vazgeç",
        confirm: "Sil"
      },
      files: "Dosyalar",
      comments: {
        title: "Yorumlar",
        placeholder: "Yorumunuzu yazın...",
        submit: "Yorum Yap",
        submitting: "Gönderiliyor...",
        count: "{{count}} Yorum"
      }
    },
    errors: {
      loadFailed: "Notlar yüklenirken bir hata oluştu",
      shareFailed: "Not paylaşılırken bir hata oluştu",
      deleteFailed: "Not silinirken bir hata oluştu",
      commentFailed: "Yorum gönderilirken bir hata oluştu",
      requireLogin: "Yorum yapmak için giriş yapmalısınız",
      emptyComment: "Yorum boş olamaz",
      noPermission: "Bu notu silme yetkiniz yok",
      selectFaculty: "Lütfen bir fakülte seçin",
      selectDepartment: "Lütfen bir bölüm seçin"
    }
  }
}; 